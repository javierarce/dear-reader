...
class Reader {
  ...
  async getWeatherDescription () {
    let weatherData = await Weather.get()
    ...
    return `Scattered clouds. The temperature in Madrid is ${temperature}ºC (feels like ${feelsLike}ºC). Humidity: ${humidity}%. Sunset time is ${sunset}.`
  }
  ...
  generateEPUB () {
    return new Promise(async (resolve, reject) => {
      ...
      contents += '<div class="Intro">'
      content += `<p>${weather}</p>`
      content += `<p>This book was sent on ${new Date().toLocaleTimeString()}</p>`
      content += '</div>'
      ...
      this.articles.forEach((article) => {
        ...
        contents += `<h2>${title} by ${author}</h2>`
        contents += `<p>${summary}</p>`
      })

      this.book.addSection('In this edition…', contents, false, false)
